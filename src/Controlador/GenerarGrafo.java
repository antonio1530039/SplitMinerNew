...
public class GenerarGrafo {
   ...
   public void computeGraph(LinkedHashMap tracesList, LinkedHashMap<String, Integer> WFG) {
      ...
      ArrayList<ArrayList<Character>> trazas = new ArrayList<ArrayList<Character>>(tracesList.values());
      System.out.println("\t1. Generando grafo a partir de log con '" + trazas.size() + "' trazas.");
      
      for (int i = 0; i < tracesList.size(); i++) {
         //procesa cada una de las listas
         ArrayList<Character> traza = trazas.get(i);
